...
| 阶段三（商店/经济/羁绊/升星/装备/存档） | 已完成 | 全部实现：金币、商店、羁绊、升星、装备、存档            |
| 阶段四（扩展） | 已完成 | 利息机制、连败补偿、装备合成树、弹道特效                         |
...
单位通过 `findNearestEnemy()` 选择攻击目标，严格以欧氏距离平方作为第一优先级，距离越近越优先。若多个敌人距离相同，则按照 PA 要求依次比较：当前生命值更高优先、横坐标更小优先（从左到右）、纵坐标更大优先（从下到上）。目标死亡后，单位会在下一次 `combatTick()` 中重新索敌。
...
**商店**：`rollShopFor()` 从商店池随机选取，当前 `kShopSlotCount = 5`。购买扣除对应金币，单位进入备战区并触发升星检查。刷新商店花费 4 金币（`kShopRefreshCost = 4`）。

### 4.7 人口系统

- 人口上限 = 玩家等级（`m_level`），初始为 3，符合 PA 对初始人口 3-4 的要求。
...
| 护卫 | Role | 2 / ≥3 | 护卫单位获得 120 / 220 护盾 |
| 神盾使 | Role | ≥2 | 神盾使单位获得 180 护盾 |

**机制改变类（1 种）**：

| 羁绊 | 类型 | 激活人数 | 效果 |
|------|------|----------|------|
| 枪手 | Role | ≥2 | 枪手单位有 35% 概率追加一次普攻 |

实现位于 `computeActiveSynergies()` 与 `performAttack()`：前者负责统计并施加数值类 buff，后者在普攻时读取枪手羁绊并触发额外攻击。羁绊仅在战斗副本上生效，不影响备战阵容。（区别于金铲铲，本游戏棋盘上同名英雄重复计数）
...
| 锁子甲 | `EquipmentType::ChainVest` | HP +150 |
...
其中铁剑（IronSword, ATK+15）、锁子甲（ChainVest, HP+150）、急速手套（HasteGloves, 攻速+20%）、蓝水晶（BlueCrystal, 最大法力-30）严格覆盖 PA 文档要求的 4 种基础装备类型。
...
| 荆棘背心 | 锁子甲 + 锁子甲 | 由两件锁子甲提供生命值，并获得反弹普攻伤害与降低暴击伤害的被动 |
...
- `findNearestEnemy()`：按 PA 规则选择欧氏距离最近的敌人，并按生命值、横坐标、纵坐标处理平局。
...
1. **启动游戏**：游戏启动后自动进入准备阶段，玩家获得初始金币 6、初始人口上限 3 和 1 个随机 1 费英雄。
2. **商店购买**：左侧商店区域显示 5 个当前可购买的英雄。点击英雄卡片扣除对应金币，英雄进入备战区。点击 "Refresh 4 gold" 按钮花费 4 金币刷新商店。
...
**商店槽位设计说明**：当前商店每次刷出 5 个英雄，与 PA 要求一致。

**与 PA.pdf 的关键对齐点**：
- 初始人口上限为 3，满足 PA 对 3-4 初始人口的要求。
- 商店固定 5 个招募位，购买后进入 8 格备战区。
- 指定基础装备已按 PA 名称和效果实现：铁剑 ATK+15、锁子甲 HP+150、急速手套攻速+20%、蓝水晶最大法力-30。
- 索敌规则以欧氏距离最近为主，距离相同时按生命值、从左到右、从下到上的顺序处理平局。
- 阶段四扩展选择了利息/连败经济、装备合成树和弹道特效，并在 GUI 中提供展示入口。
...
| 索敌规则（距离+平局优先级） | 已完成 | `game.cpp` findNearestEnemy() | 观察单位优先攻击最近目标，距离相同再按生命值和坐标处理 |
...
| 商店招募位 | 已完成 | `game.cpp` `kShopSlotCount=5` | 界面显示 5 个招募位并支持购买 |
...
| ≥2 种属性光环类羁绊 | 已完成 | 德玛西亚(HP)、艾欧尼亚(攻速)、弗雷尔卓德(双抗) | 查看激活时的属性变化 |
| ≥1 种机制改变类羁绊 | 已完成 | 枪手(35% 概率追加一次普攻) | 查看枪手单位的连射效果 |
...
本项目使用 Codex CLI 进行辅助开发。AI 主要用于需求拆解、代码审阅、局部实现建议和文档润色；核心玩法逻辑由本人理解后整合、修改和调试，验收时可解释主要模块的实现原理。

以下说明 AI 在本项目中的主要辅助范围：

**项目架构与模块拆分**：
- AI 帮助将 PA 要求拆分为棋盘/备战区、单位模型、战斗循环、寻路、技能多态、经济系统、羁绊与存档等模块。
- 本人按模块逐步实现，并在每次改动后通过编译、运行和静态检查验证。

**棋盘与 GUI 层**：
- AI 辅助整理了部分 Qt 界面展示方案，例如商店卡片文字排版、状态栏信息组织和装备说明弹窗。
- 具体交互逻辑（拖拽、购买、刷新、保存、读档、开始战斗）由本人根据游戏状态接口接入并调试。

**战斗系统**：
- AI 辅助检查了 `combatTick()` 的状态机推进顺序，使 Dead、Idle、Moving、Attacking、Casting 等状态之间的转换更清晰。
- AI 对 BFS 寻路代码给出过重构建议，本人根据棋盘占用规则保留了防重叠、不可越界和回溯第一步移动的逻辑。
- AI 辅助检查了技能多态调用链，本人确保战斗副本仍创建对应英雄派生类，避免 `Unit*` 调用落到基类空技能实现。
...
- **GUI 美术资源**：当前已为 12 个英雄添加原创头像图标，但未制作完整动画精灵序列。
- **平衡性**：英雄数值和羁绊效果未经过系统性平衡测试，当前优先保证 PA 功能闭环。
