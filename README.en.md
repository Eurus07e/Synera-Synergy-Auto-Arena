...
- `src/core/board.h` / `src/core/board.cpp`  
  The 8 x 8 board data structure. It manages grid occupation, valid positions, and player/enemy board regions.
...
The base class `Unit` defines common attributes such as HP, attack damage, attack range, mana, attack speed, armor, magic resistance, star level, equipment, owner, combat state, origin tags, and role tags.
...
Concrete hero classes inherit from `Unit` and override this function to implement different skill effects. The combat system can call:
...
Units automatically select enemies during combat. The target selection logic is based on distance, so each unit tries to find a nearby valid enemy target and then uses movement or attacks according to range.
